...
`grails-sentry` is a Grails 3.x plugin that integrates the official Sentry Java SDK (`io.sentry:sentry-logback:8.41.0`) with Grails applications. It wraps Sentry's Logback appender, enriches events with Grails/Spring context, and provides optional Spring Security user tracking.

- **Grails version**: 3.3.8
- **Sentry SDK**: sentry-logback 8.41.0
- **Plugin version convention**: mirrors Sentry SDK version (e.g., `18.41.0` for SDK `8.41.0`)
...
- SDK v8 options exposed through config: `debug`, `diagnosticLevel`, `dist`, `sendDefaultPii`, `attachThreads`, `maxRequestBodySize`, `sampleRate`, `profilesSampleRate`, `traceOptionsRequests`, `enableBackpressureHandling`, `forceInit`, `strictTraceContinuation`, `orgId`, `contextTags`, `ignoredErrors`, `ignoredTransactions`, `ignoredSpanOrigins`
- `sentryLogsEnabled` enables Sentry SDK structured Logs from Logback at or above `sentryLogsMinimumLevel`; this is separate from error event capture via `levels` and breadcrumbs via `breadcrumbsEnabled`
- `logHttpRequest` and `logClassName` default to `false`; request capture is not implemented with `sentry-logback` alone, and logger/class name capture is already automatic
- `traceOptionsRequests` defaults to `false`; the plugin's manual request tracing skips `OPTIONS` requests unless explicitly enabled
...
- Extends `SentryAppender` from `sentry-logback:8.41.0`
- Filters events by configured log levels (defaults to ERROR and WARN)
- Sends structured Sentry logs when `sentryLogsEnabled=true` without converting lower-level logs into Sentry error events
...
- Implements Sentry's `EventProcessor` interface (`process(SentryEvent, Hint)`)
...
- **`sanitizeStackTrace`** is implemented via an `EventProcessor` registered in `SentryOptions`. It calls `StackTraceUtils.isApplicationClass(frame.module)` to filter Groovy/Spring internal frames from `SentryException` stack frames. The old `toStackTraceElements` hook does not exist in modern `SentryAppender`.
...
- **`logClassName`** is always-on in the Sentry Java SDK; `SentryAppender` automatically captures the logger name (which is the class name).
